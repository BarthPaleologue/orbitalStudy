...
let bodies: BodyNewton[] = [];

// SUN
let sunMass = 2000000;
let sun = new BodyNewton(sunMass, new Vector2(0, 0), new Vector2(0, 0), null, 20, "yellow");
bodies.push(sun);
...
for (let i = 1; i < getRandomInt(3, 5); i++) {
    ...
    let body = new BodyNewton(mass, initialPosition, initialVelocity, sun, 7);
    bodies.push(body);
    ...
    for (let i = 0; i < 1; i++) {
        ...
        bodies.push(new BodyNewton(0.001, initialSatellitePosition, initialSatelliteVelocity, body, 4));
    }
}
